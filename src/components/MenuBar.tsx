...
function MenuBar () {
    const menuData = [
        {id:1, label:'홈', icon:<IoMdHome className="w-5 h-5"/>},
        {id:2, label:'검색', icon:<IoIosSearch className="w-5 h-5"/>},
        {id:3, label:'마이페이지',icon:<IoMdPerson className="w-5 h-5"/>}
    ]
    ...
    return (
        ...
        {menuData.map((data)=>{
            const isActive = data.label === active ? 'text-[#FF5160]' : 'text-[#777777]'
            return (
